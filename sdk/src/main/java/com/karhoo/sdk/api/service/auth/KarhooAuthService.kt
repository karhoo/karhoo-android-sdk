...
class KarhooAuthService : AuthService {
    ...
    override fun login(token: String): Call<UserInfo> = AuthLoginInteractor(credentialsManager = credentialsManager,
                                                                            apiTemplate = apiTemplate, userManager = userManager,
                                                                            paymentsService = KarhooApi.paymentsService)
            .apply {
                this.token = token
            }

    override fun loggedIn(credentials: Credentials): Call<UserInfo> = AuthLoggedInInteractor(credentialsManager = credentialsManager,
                                                                                             apiTemplate = apiTemplate, userManager = userManager,
                                                                                             paymentsService = KarhooApi.paymentsService)
            .apply {
