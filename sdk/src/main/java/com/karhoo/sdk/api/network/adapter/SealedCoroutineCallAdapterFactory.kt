...
import com.karhoo.sdk.api.network.client.RequestInterceptor.Companion.CORRELATION_ID
...
class SealedCoroutineCallAdapterFactory private constructor(private val analyticsManager: Analytics) :
    CallAdapter.Factory() {

    companion object {
        @JvmStatic
        @JvmName("create")
        operator fun invoke(analyticsManager: Analytics) =
            SealedCoroutineCallAdapterFactory(analyticsManager)
    }
    ...
    private class ResponseCallAdapter<T>(
        ...
        private val analyticsManager: Analytics
    ) : CallAdapter<T, Deferred<Resource<T>>> {
            ...
            call.enqueue(object : Callback<T> {
                override fun onFailure(call: Call<T>, t: Throwable) {
                    ...
                    analyticsManager.fireEvent(
                        Event.REQUEST_ERROR,
                        Payloader.Builder.builder.requestError(t.localizedMessage, url).build()
                    )
                }
                    ...
                    if (response.isSuccessful) {
                        response.body()?.let {
                            deferred.complete(
                                Resource.Success(
                                    data = it,
                                    correlationId = response.headers().get(CORRELATION_ID)
                                )
                            )
                        } ?: run {
                        ...
                        deferred.complete(
                            Resource.Failure(
                                error = KarhooError.fromThrowable(HttpException(response)),
                                correlationId = response.headers().get(CORRELATION_ID)
                            )
                        )
                    }
                    ...
                    if (response.isSuccessful) {
                        response.body()?.let {
                            ...
                            if (response.code() == HttpURLConnection.HTTP_NO_CONTENT
                                ...
                                deferred.complete(Resource.Success(void() as T, correlationId = response.headers().get(CORRELATION_ID)))
                            }
                        }
                    } else {
                        deferred.complete(
                            Resource.Failure(
                                error = KarhooError.fromThrowable(
